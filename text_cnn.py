...
class TextCNN(object):
    ...
    def __init__(self, sequence_length, num_classes,embedding_size, filter_sizes, num_filters, l2_reg_lambda=0.0):
        ...
        self.embedded_chars_expended = tf.expand_dims(self.embedded_chars, -1)#[2,186,128,1]
        ...
        for i, filter_size in enumerate(filter_sizes):
            # print (filter_sizes)[3,4,5]
            # print (i)
            with tf.name_scope("conv-maxpool-%s" % filter_size):
                # Convolution layer
                filter_shape = [filter_size, embedding_size, 1, num_filters]
                W = tf.Variable(tf.truncated_normal(filter_shape, stddev=0.1), name="W")#[3,128,1,100]
                b = tf.Variable(tf.constant(0.1, shape=[num_filters]), name="b")
                conv = tf.nn.conv2d(self.embedded_chars_expended,W,strides=[1,1,1,1],padding="VALID",name="conv")#(2, 184, 1, 100)
        ...
        with tf.name_scope("output"):
            ...
            initializer = tf.contrib.layers.xavier_initializer())#tf.contrib.layers.xavier_initializer()：一种经典的权值矩阵的初始化方式；
